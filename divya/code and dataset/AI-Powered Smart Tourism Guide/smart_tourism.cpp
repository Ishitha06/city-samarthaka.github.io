...
int main(){
    ...
    Trie tagIndex;
    unordered_map<int,Attraction> attractionMap;
    ...
    for(int i=1;i<roadsCSV.size();i++)
        maxNode=max(maxNode,max(stoi(roadsCSV[i][0]),stoi(roadsCSV[i][1])));
    ...
    /* --------------------------
       SCORE & RANK ATTRACTIONS
       -------------------------- */
    priority_queue<pair<int,int>> recPQ; // score, attraction id
    for(auto &p:attractionMap){
        int id=p.first;
        auto &a=p.second;
        int score = a.rating*10 + a.popularity - dist[id];
        recPQ.push({score,id});
    }

    cout << "Top Recommended Attractions:\n";
    int k=5;
    while(k-- && !recPQ.empty()){
        auto [score,id]=recPQ.top(); recPQ.pop();
        cout << attractionMap[id].name
             << " | Score: " << score
             << " | Distance: " << dist[id] << endl;
    }
